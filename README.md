...
In contrast, for Full Incremental Mode, as a single solver instance is maintained throughout the search process, both paths (along the branches ``x<10`` and ``x>=10``) will be encoded in the same instance via the use of activation variables. Say, activation variable ``alpha_1`` is used for path along ``x<10``. Hence, the following conditions get appended to the solver instance:
